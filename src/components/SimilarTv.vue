<template>
  <the-carousel v-if="items.length">
    <div class="movie-card" v-for="item in items" :key="item.id">
      <router-link :to="{ name: 'tv', params: { id: item.id } }">
        <div class="rate">
        ...
        />
        <div class="info">
          <h1 class="movie__title">{{ item.name }}</h1>
        </div>
      </router-link>
    </div>
  </the-carousel>
...
export default {
  ...
  methods: {
    TopRated() {
      ...
      fetch(
        `
https://api.themoviedb.org/3/tv/${movieId}/similar?api_key=${this.apikey}&language=en-US&page=${this.page}`
      )
        .then((res) => res.json())
        .then((data) => {
